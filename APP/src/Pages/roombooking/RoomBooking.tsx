...
const RoomBooking: React.FC = () => {
  const { building, roomId } = useParams<{ building: string; roomId?: string }>();
  ...
  const renderRoomList = () => {
    ...
    const roomData = [
      { id: 101, name: 'Room 101', capacity: 50, location: '1st Floor', image: 'room101.jpg' },
      { id: 102, name: 'Room 102', capacity: 30, location: '2nd Floor', image: 'room102.jpg' },
      { id: 103, name: 'Room 103', capacity: 40, location: '3rd Floor', image: 'room103.jpg' },
    ];

    return roomData.map((room) => (
      <Card key={room.id} title={room.name} style={{ width: 300, margin: '16px' }}>
    ...
    <div>
      ...
      {selectedRoom && (
        <div>
          ...
          <h2>Room Details</h2>
          <Card title={selectedRoom.name} style={{ width: 300, margin: '16px' }}>
