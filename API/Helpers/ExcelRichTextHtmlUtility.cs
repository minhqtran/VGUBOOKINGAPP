    ...
    public class ExcelRichTextHtmlUtility
    {
        ...
        public static void SetRichTextFromHtml(ExcelRange range, string html, string defaultFontName, short defaultFontSize)
        {
            ...
            foreach (Match m in System.Text.RegularExpressions.Regex.Matches(html, @"<(/?[a-z]+)[^<>]*>([\s\S]*?)(?=</?[a-z]+[^<>]*>|$)", RegexOptions.Compiled | RegexOptions.IgnoreCase))
            {
                if (isFirst)
                {
                    ...
                    thisrt.FontName = defaultFontName;
                    //thisrt.Color = System.Drawing.Color.Red; // Set the default font name
                    isFirst = false;
                }
                ...
                string value = "";
                var list_split = text.Split(" ");
                foreach (var itemss in list_split)
                {
                    value += itemss + " " + "\n";
                }
                if (thisrt.Text == "")
                ...
                else
                {
                    ...
                    // alignment.
                    thisrt = range.RichText.Add(value);
                }
        ...
        private static void SetStyleFromTag(string tag, ExcelRichText settings)
        {
            switch (tag.ToLower())
            {
                ...
                case "f":
                case "font":
                    settings.Color = System.Drawing.Color.Red;
                    break;
                case "s":
                case "strike":
